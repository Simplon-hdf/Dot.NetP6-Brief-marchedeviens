...
namespace MarcheEtDevient.Server.Controllers
{
    ...
    {
        private readonly IRepository<PublicationActu, string> _repository;

        public PublicationActusController(IRepository<PublicationActu, string> repository)
        {
        ...
        {
            var publicationActus = await _repository.GetAll();      // recupere depuis le repository toute les donnée 
        ...
        {
            var publicationActu = await _repository.GetById(id);    // recupere les donnée depuis le repository
        ...
        {
            var result = await _repository.Add(publicationActu);                                                                    // envoie vers le repository l'objet et stock le boolean de retour
        ...
        public async Task<IActionResult> UpdatePublicationActu(string id, PublicationActu publicationActu)
        {
            ...
            var result = await _repository.Update(publicationActu, id);     // envoi ves le repository l'objet a update et sont id
            if (result)
            {
